...
def boundary_conditions(r,w,p):
    ...
    if r==0:
        ...
        ksi=1.0
        delh=-1.0/p.Rs*p.dh0(1.0)
        par=3.0*p.dh0(1.0)*ksi+p.n*p.Rs*p.Rs*w*w+p.n*ddh0*ksi-p.l*(p.l+1)*p.dh0(1.0)*delh/(p.Rs*w*w)
        dksi=-1.0/(p.n+1)/p.dh0(1.0)*par-(p.l-2)*ksi
        ddelh=p.Rs*w*w*ksi-p.l*delh
    else:
...
def deriv(r,y,w,p):
    ...
    if (r==0 or r==1):
        ...
        ddelh=p.Rs*w*w*ksi/r-p.l*delh/r
        dksi=-(p.l+1)*ksi/r-p.n*p.Rs/p.h0(r)*r*delh-ksi*p.n*p.dh0(r)/p.h0(r)+p.l*(p.l+1)*delh/(p.Rs*w*w*r)
...
def dat_output(p,mode=0):
    ...
    print("결과가 'problem3.dat'로 저장되었습니다.")
    f.close()

def plot_graph(p):
    '''
    3-(2)번 문제를 그래프로 출력하는 함수
    '''
    r_values=np.linspace(0,1,GRID_POINTS)
    w=find_w(p)
    ksi_list=[]
    ...
    plt.legend()
    plt.grid(True)
    plt.title(r"3(2) $\hat{\xi}$ vs $\hat{r}$")
    plt.savefig('problem3-2ksi.png')
    print("그래프가 'problem3-2ksi.png'로 저장되었습니다.")
    plt.close()
    ...
    plt.xlabel(r"$\hat{r}$")
    plt.ylabel(r"$\hat{\delta h}$")
    plt.legend()
    plt.grid(True)
    plt.title(r"3(2) $\hat{\delta h}$ vs $\hat{r}$")
    plt.savefig('problem3-2delh.png')
    print("그래프가 'problem3-2delh.png'로 저장되었습니다.")
    plt.close()

def cal_h_amplitude(p,m):
    N=101
    grid_values=np.linspace(-1,1,N)
    dx=grid_values[1]-grid_values[0]
    dV=dx*dx*dx

    w=find_w(p)
    
    for k in range(2):
        ksi,delh=get_ksi_delh_two(w[k],p)
        delh_val=interp1d(np.linspace(0,1,GRID_POINTS),delh,kind='cubic')
        labels=["fundamental","1st overtone"]
        print(f"\n- {labels[k]} mode h amplitude calculation -")
            ...
            for y in grid_values:
                for z in grid_values:
                    ...
                    if r<=1.0 and r>0.0:
                        ...
                        rho0=(max(h0,0)/(p.K*(p.n+1)))**p.n
                        delrho0=p.n*rho0*delh_val(r)/h0
                        Ylm=sph_harm_y(int(p.l),int(m),theta,phi).real
                        
                        pos=np.array([x,y,z])

                        for i in range(3):
                            for j in range(3):
                                I[i][j]+=float(delrho0)*pos[i]*pos[j]*float(Ylm)*dV
        I_mag=np.linalg.norm(I)
        h_final = (2.0 / 2.0894e16) * (w[k]**2) * (I_mag * (p.Rs**5)) * (6.7727e-6 / p.Rs)
        print(f"h amplitude = {h_final:.6e}")

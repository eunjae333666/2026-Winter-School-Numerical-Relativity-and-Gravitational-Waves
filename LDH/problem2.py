...
def get_ksi_delh(w,p):
    '''
    최종 ksi와 delh값 구하는 함수
    '''
...
def get_ksi_delh_inv(w, p):
    '''
    표면(r=1)에서 중심(r=0)으로 적분하여 최종 ksi와 delh값을 구하는 함수
    '''
    # 1. r값을 1에서 0으로 거꾸로 배열 생성
    r_inv = np.linspace(1, 0, GRID_POINTS)
    ksi_inv = np.zeros(GRID_POINTS)
    delh_inv = np.zeros(GRID_POINTS)

    # 2. 초기값 설정: 표면(r=1) 경계 조건
    # boundary_conditions(1, w, p)에서 [ksi, delh]만 추출
    ksi_inv[0], delh_inv[0] = boundary_conditions(1, w, p)[::2]

    # 3. 중심을 향해 역방향 적분 수행
    for i in range(1, GRID_POINTS):
        r_curr = r_inv[i-1]
        y_curr = np.array([ksi_inv[i-1], delh_inv[i-1]])
        
        # 거꾸로 이동하므로 스텝 사이즈는 -dr
        ksi_inv[i], delh_inv[i] = RK2_step(r_curr, y_curr, w, p, -dr)

    # 4. 반환 시에는 다시 0에서 1 순서로 뒤집어서 전달 (그래프 그리기용)
    return ksi_inv[::-1], delh_inv[::-1]
...
def dat_output(p):
    ...
    w=find_w(p)
    f.write(f"{p.M:.8f} {p.Rs:.8f} {w[0]:.8f}\n")
    r_values=np.linspace(0,1,GRID_POINTS)
    ksi_value,delh_value=get_ksi_delh_two(w[0],p)
...
def main():
    ...
    plot_graph(p=replace(p,n=1.0,K=100))

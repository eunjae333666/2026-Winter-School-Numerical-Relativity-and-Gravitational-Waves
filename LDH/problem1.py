...
def get_h_dphi(Rs,M,p):
    '''
    최종 h, dphi/dr 값 구하는 함수
    '''
    r_values=np.linspace(0,1,GRID_POINTS)
    h_values=np.zeros(GRID_POINTS)
    dphi_value=np.zeros(GRID_POINTS)
    h_values[0],dphi_value[0]=boundary_conditions(0,Rs,M,p)[::2]
    for i in range(1,GRID_POINTS):
        y=np.array([h_values[i-1],dphi_value[i-1]])
        h_values[i],dphi_value[i]=RK2_step(r_values[i-1],y,Rs,M,p,dr)
    return h_values,dphi_value
...
def main():
    ...
    plot_graph2(p=replace(p,n=1.5))
    plot_graph3(p=replace(p,n=1.5))
